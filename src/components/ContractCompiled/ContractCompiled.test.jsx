...
it("renders Contract name", () => {
  ...
  act(() => {
    render(<ContractCompiled />, container);
  });
  expect(container.textContent).toBe("Contract Name: ")

  act(() => {
    render(<ContractCompiled contractName={contractName} />, container);
  });
  expect(container.querySelector("span").textContent).toBe("Contract Name: Strings")

  act(() => {
    render(<ContractCompiled bytecode={bytecode} />, container);
  });
  expect(container.querySelector(".byte-code").textContent).toBe(JSON.stringify(bytecode))

  act(() => {
    render(<ContractCompiled abi={abi} />, container);
  });
  expect(container.querySelector(".abi-definition").textContent).toBe(JSON.stringify(compiled.contracts[fileName][contractName].abi))
});