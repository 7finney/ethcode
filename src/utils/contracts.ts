...
const getERC4907Contract = (_jsonPayload: any): CompiledJSONOutput => {
  const path_ =
    workspace.workspaceFolders !== undefined &&
    workspace.workspaceFolders[0].uri.fsPath;
  const data = JSON.parse(JSON.stringify(_jsonPayload));
  const output: CompiledJSONOutput = {
    name: "ERC4907",
    path: path.join(path_.toString() as string, "artifacts", "contracts"),
    contractType: 3,
    ERCOutput: data,
  };
  logger.log("Loaded ERC4907 compiled JSON output.");
  return output;
};

const createERC4907Contract = (context: ExtensionContext) => {
  const contractABI = ABI;
  const contract: CompiledJSONOutput = getERC4907Contract(contractABI);
  context.workspaceState.update("contract", contract);
  createConstructorInput(contract);
  createFunctionInput(contract);
  createDeployed(contract);
